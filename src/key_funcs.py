...
def key_gen(args, endpoint_name, uuid):
    ...
    cmd =   f"""
            ...
            sleep 5
            cat "$HAPROXY_CONFIG_PATH/server.crt" "$HAPROXY_CONFIG_PATH/server.key" > "$HAPROXY_CONFIG_PATH/stunnel.pem"; chmod 600 "$HAPROXY_CONFIG_PATH/stunnel.pem"
            cat "$HAPROXY_CONFIG_PATH/stunnel.pem"
...
def key_dist(args, endpoint_name, uuid, key, crt):
    ...
    #just a double check. remove the double checks later! (another one in outbound)
    ...
    cmd =   f"""
            ...
            cat "{crt}" {key} > "$HAPROXY_CONFIG_PATH/stunnel.pem"; chmod 600 "$HAPROXY_CONFIG_PATH/stunnel.pem"
            '
            """

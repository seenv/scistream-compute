...
class Config:
    ...
    #Generals
    _RUN_NUM = 10
    _PROXY = ['StunnelSubprocess.v1.2']
    #_PROXY = ['StunnelSubprocess.v1.2_crypt', 'HaproxySubprocess', 'StunnelSubprocess.v1.3', 'Nginx', 'StunnelSubprocess.v1.2']        #can't have stunnels after each other as the proxy check will find stunnel and can't find the version
    _CONGESTIONS = ['cubic', 'bbr']
    #_HOME_DIR = f"~/{_APP.upper()}-TEST/{datetime.now().strftime('%Y-%m-%d')}"
    ...
    _MERROW_GLOBUS_SCRIPT = "/home/seena/Projects/scistream-compute/src/main.py"
    _SRC_DIR = os.path.expanduser('~/code')
    _C2CS_IP = '10.52.1.30'
    _P2CS_IP = '192.5.87.71'
    _BASE_PORT = 5100
    _LOCAL_SYS_SCRIPT = '/home/seena/Projects/scistream-compute/chameleon/src/sys_monitor.py'
    _LOCAL_DOCKER_YML = '/home/seena/Projects/scistream-compute/chameleon/mini-apps'

...
from .s2ds.utils import get_config_path
...
class S2CS(scistream_pb2_grpc.ControlServicer):
    ...
    def update(self, request, context=None):
        ...
        #deb: getting the resource map info in s2c in update
        entry = self.resource_map[request.uid]
        with open(Path(get_config_path()) / "resource.map", "a") as f:
            print("Request UID:", request.uid, file=f)
            print("Role:", entry["role"], file=f)
            print("Number of Connections:", entry["num_conn"], file=f)
            print("Rate:", entry["rate"], file=f)
            print("Listeners:", entry["listeners"], file=f)
            print("Prod Listeners:", entry["prod_listeners"], file=f)
            print(f"\n{self.resource_map[request.uid]}", file=f)
...
def start(
    ...
    try:
        ...
        #deb: getting the resource map info in s2c in update
        with open(Path(get_config_path()) / "resource.map", "w") as f:
            print("Sync Port:", port, file=f)

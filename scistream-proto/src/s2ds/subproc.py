...
class AbstractSubprocess():
    ...
    def generate_config(self, uid, dest_array, role):
        ...
        key_filename = Path(get_config_path()) / f"{uid}.key"           #deb: should keep track of this so we know which one is the current config file to  extract the info from
        pid_filename = Path(get_config_path()) / f"{uid}.pid"           #deb: basically we need the uuid 
        
        config_content = template.render(
            dest_array=dest_array,                                      #deb: or we can simply just get the dest array and uuid
            local_ports=self.local_ports,
            client="yes" if role == "CONS" else "no",
            key_filename=str(key_filename),
            pid_filename=str(pid_filename),
        )
        config_path = Path(get_config_path()) / f"{uid}.conf"           #deb: should keep track of this so we know which one is the current config file to  extract the info from

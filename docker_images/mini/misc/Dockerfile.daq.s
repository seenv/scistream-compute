...
RUN conda install -y -c conda-forge hdf5=1.14.3=mpi* mpich compilers vim      
RUN pip install --upgrade pip
RUN pip install astropy olefile flatbuffers

RUN git clone https://github.com/google/flatbuffers.git
WORKDIR /flatbuffers
#RUN cmake -G "Unix Makefiles"       
RUN cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX=$CONDA_PREFIX .
RUN make -j2
RUN make test && ctest
RUN make install
...
#RUN export CPLUS_INCLUDE_PATH=$CPLUS_INCLUDE_PATH:/usr/local/include:/opt/conda/include
#RUN cmake -DCMAKE_C_COMPILER=$(which mpicc) \

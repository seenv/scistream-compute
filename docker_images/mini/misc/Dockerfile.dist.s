...
RUN conda install -c conda-forge hdf5=1.14.3=mpi* mpich 
# so you are sure that is has parallel activated        
RUN conda install -c conda-forge compilers                      
# conda-installed MPICH compiler needs its compiler

RUN pip install --upgrade pip
RUN pip install astropy olefile                                 
# some versions dont install it automatically

RUN git clone https://github.com/google/flatbuffers.git
WORKDIR /flatbuffers
RUN cmake -G "Unix Makefiles"       
#cmake -G Unix Makefiles -DCMAKE_INSTALL_PREFIX=$CONDA_PREFIX 
RUN make -j
RUN make install
